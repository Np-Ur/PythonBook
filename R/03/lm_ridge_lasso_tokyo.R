...
# Lasso回帰
lasso <- train(data=train_data, 取引価格.総額. ~ ., method="glmnet", tuneGrid = expand.grid(alpha = 1, lambda = 1))
y_pred <- predict(lasso, test_data)
MAE(y_pred, test_data$取引価格.総額.)


# Ridge回帰
ridge <- train(data=train_data, 取引価格.総額. ~ ., method="glmnet", tuneGrid = expand.grid(alpha = 0, lambda = 1))
